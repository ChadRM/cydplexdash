...
// on the home LAN and still works away from home via Tailscale Funnel. Only used to fetch cover
// art directly from Plex's photo transcoder - session/watch-history data comes from Tautulli
// instead (see tautulli_api.h), for its cleaner per-user display names. `jsonAccept` adds an
